...
class TensorBoardLogger(TensorBoardWriterWrapper):
    ...
    def log_scalars(self, name: str, data: Dict[str, Scalar], step: int) -> None:
        """Plot multiple series on a single TensorBoard chart via add_scalars."""
        if not self._writer:
            return
        self._writer.add_scalars(name, data, global_step=step)
...
class Logger:
    ...
    def log_scalars(self, name: str, data: Dict[str, Scalar], step: int) -> None:
        if self.tb_logger:
            self.tb_logger.log_scalars(name, data, step)
...
def setup_logging(
    ...
    # By default keep the console quiet (WARNING+) so tqdm bars are uncluttered.
    # Set SAM2_VERBOSE=1 (via --verbose flag in train.py) to restore full INFO output.
    console_level = log_level_primary if os.environ.get("SAM2_VERBOSE") else "WARNING"
