...
def _evaluate_single_tile(gt_path, pred_dir, output_dir):
    ...
    try:
        ...
        print(f"⚠️ Error evaluating {base_name}: {e}")
        return False
...
#         if not os.path.exists(pred_path):
#             # print(f"\n⚠️ Missing prediction for {base_name}. Skipping...")
#             continue
...
def inspect_evaluation_metrics(eval_dir, area_name=""):
...
def _process_single_eval_file(eval_path, gt_dir):
    ...
    try:
        ...
        print(f"⚠️ Error processing {eval_path}: {e}")
